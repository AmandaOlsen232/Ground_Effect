...
def MachUp(x):
    ...
    if d == 0:
        ...
        part_span, di = compound_di(x, t)
    
    ## update dihedral file
    f = open('dihedral.txt', 'w')
    f.write('span,dihedral\n0.0,0.0\n')
    for i in range(len(part_span)):
        f.write('{:.16f},{:.16f}\n'.format(part_span[i], di[i]))
...
def compound_di(x, t):
    ...
    for b in span_frac:
        if b0 <= b <= b1:
            ...
            partspan.append(b)
            di_dist.append(di)
    if b1 not in partspan:
        partspan.append(b1)
        di_dist.append(g1)
    
    count = 2
    for _ in range(len(di_cp)-2):
        partspan.append(di_cp[count])
        count+=1
    count = 0
    for _ in range(len(di_points)-1):
        di_dist.append(di_points[count+1])
        count += 1
    
    return partspan, di_dist
...
if __name__ == '__main__':
    ...
    all_results_path = "C:/Users/A02247969/Docs/AeroLab/Ground Effect/2_10_2025/ryzen/All_Results"

...
if __name__ == '__main__':
    ...
    file_name = "RA8.0_RT1.0_CL0.5_hbchange_cp4d5t_initprevious_LLsolvernonlinear_didistquadratic_grid100_optmethodSLSQP_dibounds(-90, 90)_opttol1e-12_optmaxiter1000_cases100_full.txt"
    #for file_name in all_txt_files:
    f = open(file_name, 'r+')
    opt_data = f.readlines()
    f.close() 
    
    if "4d5t" in file_name:
        num_lines = 24
        
    #int(len(opt_data)/num_lines)
    
    i=0
    #for i in cases:
    start = num_lines*i + 15
    end = num_lines*i + 24
    hello = opt_data[start:end]
    x = [float(val) for val in hello]
    
    coeff_index = num_lines*i + 2
    divide1 = "h/b: "
    val = opt_data[coeff_index].split(divide1)[1]
    val = float(val)
    print(coeff_index, val)
    print(x)
    MachUp(x)
    runCase(RA[0], RT[0], CL[0], val)
    dist = MachUpDist()
    b=8
    # f = open('input.json', 'r+')
    # b_data = f.readlines()
    ...
    cdi.plot(y_b, z_b, color="black", linewidth=plot_linewidth, linestyle="solid")
    ...
    cdi.plot(y_b, z_b, color="black", linewidth=plot_linewidth, linestyle="solid")
    # if "cases10" in file_name:
    #     hello = opt_data[231:240]
    # else:
    #     hello = opt_data[15:24]
